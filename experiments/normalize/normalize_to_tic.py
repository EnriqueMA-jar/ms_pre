...
def normalize_to_tic(input_path):
    ...
    base_name = os.path.basename(input_path).replace(".mzML", "_TO_TIC_normalized.mzML")
    ...
    fig.update_layout(
        title={
            'text': 'Spectrum Before Normalization to TIC',
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 16}
        },
    ...
    fig2.update_layout(
        title={
            'text': 'Spectrum After Normalization to TIC',
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 16}
        },
